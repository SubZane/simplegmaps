	...
	*/
		...
		var getMapMarkersFromJSON = function (done) {
			...
			request.onload = function () {
				if (request.status >= 200 && request.status < 400) {
					...
					if (settings.debug) {
						hook("onJSONLoadFail");
						console.log(
							"An error occured when trying to load data from " +
								settings.datasource
						);
					}
				}
			...
			request.onerror = function () {
				...
				if (settings.debug) {
					...
					console.log(
						"A connection error occured when trying to access " +
							settings.datasource
					);
				}
			};
		...
		var getMapMarkersFromMarkup = function (done) {
			...
			markerNodes.forEach(function (markerElement) {
				...
				if (markerElement.querySelector(".map-infowindow")) {
					...
					marker.CustominfoWindowContent = markerElement.querySelector(
				...
				if (marker.address === null && marker.latlng === null) {
					log(
						"No address or longitude/latitude found in markup. Removing marker."
					);
					//remove marker
					markerListlength--;
					return;
		...
		var fetchPosition = function (marker, callback) {
			if (hasValue(marker.latlng)) {
				...
				setTimeout(
					getLatLngPosition(marker, function (status, response) {
						log("status: " + status);
						...
						if (status === google.maps.GeocoderStatus.OVER_QUERY_LIMIT) {
							...
							log(status);
							log(
								"No address or longitude/latitude located from address. Removing marker."
							);
							//remove marker
							callback(false);
						}
			...
			if (markerData.infoWindowContent) {
				...
				google.maps.event.addListener(marker, "click", function () {
					...
					el.insertAdjacentHTML(
						"afterend",
						'<div id="simplegmaps-c-iw"></div>'
					);
					document.querySelector("#simplegmaps-c-iw").innerHTML =
						customInfowindow;
					document
						.querySelector("#simplegmaps-c-iw .close")
		...
		var toggleBounce = function (markerObj) {
			if (
				typeof markerObj.getAnimation() === "undefined" ||
				markerObj.getAnimation() === null
			) {
		...
		simplegmaps.Search = {
			...
			init: function (options) {
				simplegmaps.Search.options = extend(
					simplegmaps.Search.default,
					options || {}
				);
				...
				document
					.querySelector(simplegmaps.Search.options.eventButton)
					.addEventListener("click", function (event) {
						event.preventDefault();
						search(
							document.querySelector(simplegmaps.Search.options.input).value
						);
					});

				document
					.querySelector(simplegmaps.Search.options.input)
		...
		simplegmaps.Directions = {
			...
			init: function (routingOptions) {
				simplegmaps.Directions.options = extend(
					simplegmaps.Directions.default,
					routingOptions || {}
				);
				DirectionsRequest.TravelMode =
					simplegmaps.Directions.options.TravelMode;
				DirectionsRequest.UnitSystem =
					simplegmaps.Directions.options.UnitSystem;

				document
					.querySelector(simplegmaps.Directions.options.eventButton)
					.addEventListener("click", function (event) {
						simplegmaps.Directions.route({
							origin: document.querySelector(
								simplegmaps.Directions.options.originInput
							).value,
				...
				if (simplegmaps.Directions.options.AutoComplete) {
					bindAutoComplete(
						simplegmaps.Directions.options.originInput,
						simplegmaps.Directions.options.eventButton
					);
				}
			...
			route: function (directionsRequest) {
				...
				if (settings.routeDescriptionContainer) {
					directionsDisplay.setPanel(
						document.querySelector(settings.routeDescriptionContainer)
					);
				}
