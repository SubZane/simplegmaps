...
var simplegmaps = (function () {
	...
	var Constructor = function () {
		...
		*/
		...
		var getMarkerAddress = function (inmarker, callback) {
			log('getMarkerAddress:marker: ' + inmarker);
			geocoder.geocode({
				latLng: inmarker.getPosition()
			}, function (responses) {
		...
		publicAPIs.getMarkerAddress = function (marker, callback) {
			getMarkerAddress(marker, function (address) {
				callback(address);
			});
		};

		publicAPIs.Center = function () {
		...
		publicAPIs.Search = {
			init: function (options) {
				settings.Search = extend(defaults.Search, options || {});

				if (settings.Search.AutoComplete) {
					bindAutoComplete(settings.Search.input);
				}

				document.querySelector(settings.Search.eventButton).addEventListener('click', function (event) {
					event.preventDefault();
					search(document.querySelector(settings.Search.input).value);
				});

				document.querySelector(settings.Search.input).addEventListener('keypress', function (event) {
					if (event.keyCode === 13) {
						search(document.querySelector(settings.Search.input).value);
					}
				});
		...
		publicAPIs.Directions = {
			...
			init: function (routingOptions) {
				settings.Directions = extend(defaults.Directions, routingOptions || {});
				DirectionsRequest.TravelMode = settings.Directions.TravelMode;
				DirectionsRequest.UnitSystem = settings.Directions.UnitSystem;

				document.querySelector(settings.Directions.eventButton).addEventListener('click', function (event) {
					simplegmaps.Directions.route({
						origin: document.querySelector(settings.Directions.originInput).value,
						destination: settings.Directions.destination
					});
				});
				if (settings.Directions.AutoComplete) {
					bindAutoComplete(settings.Directions.originInput, settings.Directions.eventButton);
				}
		...
		publicAPIs.TrafficLayer = {
			toggle: function () {
				toggleTrafficLayer();
			}
		};

		publicAPIs.TransitLayer = {
			toggle: function () {
				toggleTransitLayer();
			}
		};

		publicAPIs.BicycleLayer = {
			toggle: function () {
				toggleBicycleLayer();
			}
		};

		publicAPIs.GeoLocation = {
			set: function () {
				geoLocation();
			}
		};
		...
		publicAPIs.getURL = {
			...
			Native: function (address) {
				var url = '';
				if (isAndroid()) {
					url = this.Android(address);
				} else if (isIOS()) {
					url = this.ios(address);
				} else if (isWindowsPhone()) {
					url = this.WindowsPhone(address);
				} else {
					url = this.Desktop(address);
				}
				return url;
			}
		};
